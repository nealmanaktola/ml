...
ff_config = {
    ...
    'mimic': {
        'iteration_list': [10000],
        'max_attempts': 100,
        'population_sizes': [500, 1500, 2000],
...
def plot_fitness_curve(rhc, sa, ga, mimic):
    ...
    plt.title('FlipFlop Fitness vs Iterations')
    plt.xlabel('Iterations')
    plt.ylabel("Fitness")
    plt.legend(loc="best")
    plt.grid()
    plt.savefig("flipflop-fitness.png")
...
def plot_time_curve(rhc, sa, ga, mimic):
    ...
    plt.title('FlipFlop Time vs Iterations')
    plt.xlabel('Iterations')
    plt.ylabel("Time")
    plt.legend(loc="best")
    plt.grid()
    plt.savefig("flipflop-time.png")
...
def plot_fevals_curve(rhc, sa, ga, mimic):
    ...
    plt.title('FlipFlop FEvals vs Iterations')
    plt.xlabel('Iterations')
    plt.ylabel("FEvals")
    plt.legend(loc="best")
    plt.grid()
    plt.savefig("flipflop-fevals.png")


def run():
    config = ff_config['rhc']
    problem = mlrose.DiscreteOpt(
        length=ff_config['size'], fitness_fn=FlipFlop(), maximize=True, max_val=2)

    problem_size_results = {}

    # Run Random Hill
    rhc_runner = RHCRunner(problem, experiment_name=f"flipflop-rhc", seed=random_seed,
    ...
    rhc_problem_results = np.zeros(len(ff_config['sizes']))
    for i in range(len(ff_config['sizes'])):
        n = ff_config['sizes'][i]
        start_time = time.time()
        prob = mlrose.DiscreteOpt(
            length=n, fitness_fn=FlipFlop(), maximize=True, max_val=2)
    ...
    config = ff_config['sa']
    sa_runner = SARunner(problem, experiment_name=f"flipflop-sa", seed=random_seed, decay_list=[
    ...
    max_iters = max(config['iteration_list'])
    max_attempts = config['max_attempts']
    sa_problem_results = np.zeros(len(ff_config['sizes']))
    for i in range(len(ff_config['sizes'])):
        n = ff_config['sizes'][i]
        start_time = time.time()
        prob = mlrose.DiscreteOpt(
    ...
    # Run GA
    config = ff_config['ga']
    ga_runner = GARunner(problem, experiment_name=f'flipflop-ga', seed=random_seed,
    ...
    max_iters = max(config['iteration_list'])
    max_attempts = config['max_attempts']
    ga_problem_results = np.zeros(len(ff_config['sizes']))
    for i in range(len(ff_config['sizes'])):
        n = ff_config['sizes'][i]
        start_time = time.time()
        prob = mlrose.DiscreteOpt(
    ...
    # Run Mimic
    config = ff_config['mimic']
    mimic_runner = MIMICRunner(problem=problem, experiment_name=f"flipflop-mimic", seed=random_seed, use_fast_mimic=True,
    ...
    max_iters = max(config['iteration_list'])
    max_attempts = config['max_attempts']
    mimic_problem_results = np.zeros(len(ff_config['sizes']))
    for i in range(len(ff_config['sizes'])):
        n = ff_config['sizes'][i]
        prob = mlrose.DiscreteOpt(
            length=n, fitness_fn=FlipFlop(), maximize=True, max_val=2)
