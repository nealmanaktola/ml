...
def run_frozen_lake_ql(size=(4, 4), value=0.99, gamma=0.9, epsilon=0.999, epsilon_decay=0.99,
    ...
    if size[0] == 4 and size[1] == 4:
        ...
        env = gym.make("FrozenLake-v1").env  # to avoid time limit
        env = TimeLimit(env, max_episode_steps=1000)
    else:
        random_map = generate_random_map(size=size[0], p=0.90)
        print(random_map)
        env = gym.make("FrozenLake-v1", desc=random_map).env
    ...
    qlearn = hiive.mdptoolbox.mdp.QLearning(
        P, R, gamma=0.9, epsilon=value, epsilon_min=0.1, n_iter=500000, iter_callback=check_if_new_episode)
    ...
    plt.figure(10)
    plt.plot(iterations, times, "-o", label=f'{value}')
    plt.xlabel("Iteration")
    plt.ylabel("Time")
    plt.title(f"Frozenlake {size_str} QL Time vs Iteration")
    plt.legend(loc="upper right")
    plt.savefig(f"epsilon_frozenlake_ql_{size_str}_time_vs_iteration.png")

    plt.figure(11)
    plt.plot(iterations, errors, "-o", label=f'{value}')
    plt.xlabel("Iteration")
    plt.ylabel("Error")
    plt.title(f"Frozenlake {size_str} QL Error vs Iteration")
    plt.legend(loc="upper right")
    plt.savefig(f"epsilon_frozenlake_ql_{size_str}_error_vs_iteration.png")

    plt.figure(13)
    plt.plot(iterations, mean_v, "-o", label=f'{value}')
    plt.xlabel("Iteration")
    plt.ylabel("Mean V")
    plt.title(f"Frozenlake {size_str} QL Mean V vs Iteration")
    plt.legend(loc="upper right")
    plt.savefig(f"epsilon_frozenlake_ql_{size_str}_meanv_vs_iteration.png")

    return stats


def run_frozen_lake(size=(4, 4), max_iter=10000, algorithm='VI', gamma=0.99):
    size_str = f"{size[0]}x{size[1]}"

    print(f"\n Running frozen lake mdp example-{size_str}")

    # Obtain P/R
    if size[0] == 4 and size[1] == 4:
        env = gym.make("FrozenLake-v1").env
    else:
        random_map = generate_random_map(size=size[0], p=0.9)
        print(random_map)
        env = gym.make("FrozenLake-v1", desc=random_map).env
        from gym.wrappers.time_limit import TimeLimit
    ...
    plt.xlabel("Iteration")
    plt.ylabel("Time")
    plt.title(f"Frozenlake {size_str} {algorithm} Time vs Iteration")
    plt.legend(loc="upper right")
    plt.savefig(f"{algorithm}_frozenlake_{size_str}_time_vs_iteration.png")

    plt.figure(11)
    plt.plot(iterations, errors, "-o", label=f'{gamma}')
    plt.xlabel("Iteration")
    plt.ylabel("Error")
    plt.title(f"Frozenlake {size_str} {algorithm} Error vs Iteration")
    plt.legend(loc="upper right")
    plt.savefig(f"{algorithm}_frozenlake_{size_str}_error_vs_iteration.png")

    plt.figure(12)
    plt.plot(iterations, reward, "-o", label=f'{gamma}')
    plt.xlabel("Iteration")
    plt.ylabel("Reward")
    plt.title(f"Frozenlake {size_str} {algorithm} Reward vs Iteration")
    plt.legend(loc="upper right")
    plt.savefig(f"{algorithm}_frozenlake_{size_str}_reward_vs_iteration.png")

    plt.figure(13)
    plt.plot(iterations, mean_v, "-o", label=f'{gamma}')
    plt.xlabel("Iteration")
    plt.ylabel("Mean V")
    plt.title(f"Frozenlake {size_str} {algorithm} Mean V vs Iteration")
    plt.legend(loc="upper right")
    plt.savefig(f"{algorithm}_frozenlake_{size_str}_meanv_vs_iteration.png")


# default args baby

epsilons = [0.9, 0.8, 0.6, 0.3]
for epsilon in epsilons:
    run_frozen_lake_ql(size=(20, 20), value=epsilon)
    # run_frozen_lake(size=(20, 20), algorithm="VI", gamma=gamma)
    #run_frozen_lake_ql(size=(20, 20), algorithm="VI", gamma=gamma)
